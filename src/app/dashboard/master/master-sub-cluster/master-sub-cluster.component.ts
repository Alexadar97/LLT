...
@Component({
  selector: 'app-master-sub-cluster',
  templateUrl: './master-sub-cluster.component.html',
  styleUrls: ['./master-sub-cluster.component.css']
})
export class MasterSubClusterComponent implements OnInit {
  p1 = 1;
  private saveSubClusterApi = this.getdata.appconstant + 'subcluster/save';
  private listSubClusterApi = this.getdata.appconstant + 'subcluster/list';
  private listClusterApi = this.getdata.appconstant + 'cluster/list';
  private GetApi = this.getdata.appconstant + 'subcluster/get';
  private seachapi = this.getdata.appconstant + 'subcluster/searchCluster';
  ...
  constructor( private http: Http,private Formbuilder: FormBuilder, private router: Router, private getdata: DatatransferService,
    ...
    this.SubclusterForm = this.Formbuilder.group({
      'id': [null],
      "subclustername": [null, Validators.compose([Validators.required])],
      "clusterid": [null, Validators.compose([Validators.required])],
    });
   }
